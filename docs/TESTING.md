...
```
 ...
 ✓ src/__tests__/NavBar.test.tsx              (6 tests)
 ...
 ✓ src/__tests__/LandingPage.test.tsx        (22 tests)
 ✓ src/__tests__/CreateVmPage.test.tsx       (39 tests)
 ✓ src/__tests__/LogsPage.test.tsx           (13 tests)
 ✓ src/__tests__/DocsPage.test.tsx           (6 tests)
 ✓ src/__tests__/VmDetailPage.test.tsx       (39 tests)
 ✓ src/__tests__/ProvisionPage.test.tsx      (46 tests)
 ✓ src/__tests__/ShareFolderPage.test.tsx    (20 tests)
 ✓ src/__tests__/ShareLogsPage.test.tsx      (17 tests)
 ✓ electron/__tests__/ipc-handlers.test.js  (119 tests)
...
| `src/__tests__/NavBar.test.tsx` | navigation links render; active tab highlighted; tab switching | 6 |
| `src/__tests__/VmRunningBadge.test.tsx` | running/stopped/starting badge states and label text | 10 |
| `src/__tests__/LogPanel.test.tsx` | log line rendering, stdout/stderr source tags, empty state | 8 |
| `src/__tests__/ProgressBar.test.tsx` | progress bar rendering and value display | 3 |
| `src/__tests__/LandingPage.test.tsx` | VM list (empty/populated), start/stop/delete buttons, running badge, navigation to detail and provision | 22 |
| `src/__tests__/DocsPage.test.tsx` | doc loading from IPC, markdown rendering, file-not-found error state | 6 |
...
| `src/__tests__/ProvisionPage.test.tsx` | `mode view` — Base Setup and By Category cards; `script list` — by-category grouping, category drill-down, script-args form; `running state` — label while in flight; `done state banners` — green success, red failure with `errorDetail`, blue already-installed, mutual exclusion, action buttons; `forceConfirm` — amber panel, Cancel, "Install anyway" passes `--force`; `changeHostname toggle` — input shown/hidden, pre-filled from `getVmHostname`; `AI Tools — Claude Code` — category visible in grid, correct scriptRelPath | 46 |
| `src/__tests__/VmDetailPage.test.tsx` | `header` — VM name heading, Back button; `loading/error states`; `General/Hardware/Network/Log sync/Shared folders sections` — field values, null placeholders, format helpers; `Installed tools tab` — Refresh button states (Checking.../Refresh/stopped), stopped/no-credentials/error messages, detected tool labels and version badges, "Nothing installed yet"; `navigation` — Share Folder and Share Logs sub-pages | 39 |
| `src/__tests__/ShareFolderPage.test.tsx` | folder picker, host-path display, share button, script running state, error/success banners | 20 |
| `src/__tests__/ShareLogsPage.test.tsx` | destination path, Sync button, script running state, error/success banners | 17 |
| `electron/__tests__/ipc-handlers.test.js` | All IPC handlers: `parseVmList`, `parseChecksOutput`, `get-downloads-path`, `open-log-dir`, `read-doc`, `load-vm-credentials`, `save-vm-credentials`, `query-vm-installed` (VM stopped, no creds, success, guestcontrol failure), `check-vm-ready`, `list-vms`, `start-vm`, `stop-vm`, `delete-vm`, `get-vm-guest-logs-path`, `get-vm-info`, `run-share-folder`, `run-share-logs`, `run-provision-script`, `run-provision-setup`, `get-vm-hostname`, `check-vm-credentials`, `run-sanity-checks`, `create-vm`, `install-virtualbox`, `log-error` | 119 |
