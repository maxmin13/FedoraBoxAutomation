...
if [[ ! -f "${CACHED_TAR}" ]]; then
    ...
    log_info "Downloading Tomcat ${TOMCAT_VERSION} from ${TOMCAT_URL} ..."
    wget -q --tries=3 "${TOMCAT_URL}" -O "${CACHED_TAR}"
...
log_info "Enable at boot: systemctl enable ${SERVICE_NAME}"
log_info "Logs         : journalctl -u ${SERVICE_NAME} -f"
log_info "Uninstall    : systemctl stop ${SERVICE_NAME} && systemctl disable ${SERVICE_NAME}"
log_info "             : rm -f /etc/systemd/system/${SERVICE_NAME}.service && systemctl daemon-reload"
log_info "             : rm -rf ${TOMCAT_DIR}"
