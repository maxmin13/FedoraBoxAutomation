...
# Tests for vm/setup/utilities.sh
#
# Run from the project root:
#   bats vm/tests/utilities.bats

SCRIPT="$(cd "$(dirname "$BATS_TEST_FILENAME")/.." && pwd)/setup/utilities.sh"
...
@test "installs dconf-editor expect gedit" {
    run bash "$SCRIPT"
    grep -q "^dnf install -y dconf-editor expect gedit" "$CALLS_FILE"
}
