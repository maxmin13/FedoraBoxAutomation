...
Lists every registered VirtualBox VM with its running state and Start / Stop / Delete buttons. Click a VM name to open its detail page.
...
Verifies that the VM is running and Guest Additions are active before allowing a mount. Enter the host path and the mount point inside the VM.
...
Current totals: *427 Vitest tests* across 16 files.
