...
![Platform](https://img.shields.io/badge/platform-Windows%2011%20Home-blue)
...
> **Why Hyper-V must be off:** VirtualBox and Hyper-V both require exclusive access to the CPU's virtualisation hardware. Running both simultaneously causes VM start failures.
...
1. Click **Setup** → **Run Analysis** to check your host
2. Click **Create VM** and follow the 4-step wizard
3. After installing Guest Additions inside the VM, click **Provision** to install dev tools
...
| Page | Description |
|------|-------------|
| **My VMs** | Lists all registered VirtualBox VMs with Start / Stop / Delete controls |
| **Setup** | Runs the environment analysis; left panel lists all checks, right panel shows detail and fix instructions for the selected check; first failing check is auto-selected |
| **Create VM** | 4-step wizard — Identity → Hardware → Options → Confirm; ISO field opens a native file picker; streams live output during creation |
| **Logs** | Views the last 500 lines of `gui.log` (IPC calls) and `host.log` (PowerShell transcript); auto-scrolls to newest entry; folder shortcuts open log directories in Explorer |
| **Docs** | Renders the project markdown docs inside the app (dev mode only) |

**State persistence:** Setup and Create VM stay mounted in the background so analysis results and wizard progress survive navigation.
...
`provision-vm.ps1` requires Guest Additions installed inside the VM. `create-vm.ps1` offers to attach the ISO automatically. After Fedora OS installation completes:
...
Full setup and usage instructions are in [docs/TESTING.md](docs/TESTING.md).

| Suite | Command | Where |
|-------|---------|-------|
| PowerShell — Pester v5 | `Invoke-Pester -Path ".\host\virtualbox-sanity-checks.Tests.ps1" -Output Detailed` | Windows PowerShell |
| Bash — bats-core | `sudo bats vm/tests/` | Linux / WSL |
| React + Electron — Vitest | `cd app && npm test` | Windows or Linux (Node.js) |
...
See [CONTRIBUTING.md](CONTRIBUTING.md) for coding standards covering PowerShell scripts, Bash scripts, and guest control patterns.
